...
function deriveScenario(base: StressScenario, values: Pick<StressScenario, 'btcMove' | 'ethMove' | 'ssiMemeMove' | 'volatilitySpike' | 'etfFlowPressure'>): StressScenario {
  const severity =
  ...
  const effectiveness = Math.round(
    clamp(45 + values.volatilitySpike * 0.045 + Math.abs(values.etfFlowPressure) * 0.08 - Math.max(0, Math.abs(values.ssiMemeMove) - 60) * 0.2, 38, 72)
  );
        ...
        <div className="space-y-6 p-4 pb-24 sm:p-6 lg:p-8">
          <header>
            ...
            <p className="mt-2 max-w-2xl font-manrope text-sm leading-6 text-text-secondary">
              Model market scenarios and see how a hedge would protect your portfolio.
            </p>
          </header>
          ...
          <div className="grid gap-6 xl:grid-cols-[1.2fr_0.8fr]">
                  ...
                  <label key={String(key)} className="block">
                    <div className="mb-2 flex items-center justify-between gap-3">
                      ...
                      <span className="rounded-full bg-accent-lime-dim px-2.5 py-1 font-mono text-xs text-accent-lime">
                        {customValues[key as keyof typeof customValues]}
                        {unit}
                      </span>
                    </div>
                    <input
                      ...
                      onChange={(event) => updateValue(key as keyof typeof customValues, Number(event.target.value))}
            ...
            >
              <DGMetricCard
                label="Exposure Without Hedge"
                value={formatPercent(scenario.estimatedDrawdownWithoutHedge)}
                subtext="Estimated portfolio drawdown"
                highlight="danger"
                trend="down"
              />
              <DGMetricCard
                label="Protection With Hedge"
                value={formatPercent(scenario.estimatedDrawdownWithHedge)}
                subtext="Estimated portfolio drawdown"
                highlight="positive"
              />
              <DGMetricCard
                label="Hedge Effectiveness"
                value={`${scenario.hedgeEffectiveness}%`}
                subtext="Drawdown reduction"
                highlight="positive"
              />
              <GlowCard className="p-5">
          ...
          <GlowCard className="p-5">
            ...
            <p className="mt-4 font-manrope text-sm leading-7 text-text-secondary">
              In a {scenario.label} scenario with BTC moving {scenario.btcMove}%, the unhedged portfolio is expected to
              experience a {formatPercent(scenario.estimatedDrawdownWithoutHedge)} drawdown, representing a loss of
              approximately{' '}
            ...
            <p className="mt-4 font-manrope text-sm leading-7 text-text-secondary">
              ...
              {formatPercent(scenario.estimatedDrawdownWithHedge)}, representing hedge effectiveness of{' '}
              {scenario.hedgeEffectiveness}%.
            </p>
            <p className="mt-4 font-manrope text-sm leading-7 text-text-secondary">
