  ...
  try {
    await db.query(`
      INSERT INTO portfolio_snapshots 
        (wallet_address, snapshot_timestamp_ms, total_usd_value)
      VALUES ($1, $2, $3)
      ON CONFLICT (wallet_address, snapshot_timestamp_ms) 
      DO UPDATE SET total_usd_value = EXCLUDED.total_usd_value
    `, [walletAddress.toLowerCase(), Date.now(), totalUsdValue]);
  } catch (e) {
    // Log but never throw — snapshot failure should not block portfolio display
    console.error("[SnapshotWriter] Failed to write snapshot:", e);
  }
}
